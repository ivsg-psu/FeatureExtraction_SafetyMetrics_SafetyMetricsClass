...
%% script_test_fcn_SafetyMetrics_findFirstTrajLaneCollison
% Exercises the function: fcn_SafetyMetrics_findFirstTrajLaneCollison
...
% Call the function
firstHitStruct = fcn_SafetyMetrics_findFirstTrajLaneCollison(vehicleTrajectoryPath, laneMarkerPath, (figNum));

sgtitle(titleString, 'Interpreter','none','Fontsize',12);

% Assertions
assert(isequal(class(firstHitStruct), 'struct'))
assert(isequal(length(firstHitStruct.lane_marker_pathXY(:,1)),length(laneMarkerPath(:,1))));
assert(~firstHitStruct.did_intersect)
assert(isempty(firstHitStruct.intersection_pathXY))
assert(isempty(firstHitStruct.intersection_traj_vector_index))
assert(isempty(firstHitStruct.intersection_traj_station))

% Make sure plot opened up
assert(isequal(get(gcf,'Number'),figNum));

%% DEMO case: Check for a collision between a vertical lane and a vehicle going in a straight path

figNum = 10002;
titleString = sprintf('DEMO case: Check for a collision between a vertical lane and a vehicle going in a straight path');
fprintf(1,'Figure %.0f: %s\n',figNum, titleString);
figure(figNum); clf;

% Generate Trajectory
vehicleTrajString = 'straight'; 
laneMarkerString = 'lane vert a'; 
[vehicleTrajectoryPath, laneMarkerPath] = fcn_INTERNAL_generateTrajAndLaneMarkerPath(vehicleTrajString, laneMarkerString);
...
assert(~isempty(firstHitStruct.intersection_traj_vector_index))
assert(~isempty(firstHitStruct.intersection_traj_station))
assert(isequal(firstHitStruct.intersection_traj_station, 50))
...
%% DEMO case: Check for a collision between a straight lane marker and a vehicle going in a more curvy path - crosses zero many times but outputs only one intersection
...
assert(~isempty(firstHitStruct.intersection_pathXY))
assert(~isempty(firstHitStruct.intersection_traj_vector_index))
assert(isequal(length(firstHitStruct.intersection_traj_vector_index(:,1)), length(firstHitStruct.intersection_pathXY(:,1))))
assert(~isempty(firstHitStruct.intersection_traj_station))
assert(isequal(firstHitStruct.intersection_traj_station, 0))
...
% Assertions 
assert(isequal(class(firstHitStruct), 'struct'))
assert(isequal(length(firstHitStruct),3))

% Assertions - Lane marker A
assert(length(firstHitStruct(1).lane_marker_pathXY(:,1)) < length(laneMarkerPath(:,1)));
assert(~firstHitStruct(1).did_intersect)
assert(isempty(firstHitStruct(1).intersection_pathXY))
assert(isempty(firstHitStruct(1).intersection_traj_vector_index))
assert(isempty(firstHitStruct(1).intersection_traj_station))


% Assertions - Lane marker B
assert(length(firstHitStruct(2).lane_marker_pathXY(:,1)) < length(laneMarkerPath(:,1)));
assert(~firstHitStruct(2).did_intersect)
assert(isempty(firstHitStruct(2).intersection_pathXY))
assert(isempty(firstHitStruct(2).intersection_traj_vector_index))
assert(isempty(firstHitStruct(2).intersection_traj_station))

% Assertions - Lane marker C
assert(length(firstHitStruct(3).lane_marker_pathXY(:,1)) < length(laneMarkerPath(:,1)));
assert(firstHitStruct(3).did_intersect)
assert(~isempty(firstHitStruct(3).intersection_pathXY))
assert(~isempty(firstHitStruct(3).intersection_traj_vector_index))
assert(isequal(length(firstHitStruct(3).intersection_traj_vector_index(:,1)), length(firstHitStruct(3).intersection_pathXY(:,1))))
assert(~isempty(firstHitStruct(3).intersection_traj_station))
...
assert(~isempty(firstHitStruct(1).intersection_pathXY))
assert(~isempty(firstHitStruct(1).intersection_traj_vector_index))
assert(isequal(length(firstHitStruct(1).intersection_traj_vector_index(:,1)), length(firstHitStruct(1).intersection_pathXY(:,1))))
assert(~isempty(firstHitStruct(1).intersection_traj_station)); 
...
assert(~isempty(firstHitStruct(2).intersection_pathXY))
assert(~isempty(firstHitStruct(2).intersection_traj_vector_index))
assert(isequal(length(firstHitStruct(2).intersection_traj_vector_index(:,1)), length(firstHitStruct(2).intersection_pathXY(:,1))))
assert(~isempty(firstHitStruct(2).intersection_traj_station));
...
% Call the function
firstHitStruct = fcn_SafetyMetrics_findFirstTrajLaneCollison(vehicleTrajectoryPath, laneMarkerPath, ([]));
...
% Call the function
firstHitStruct = fcn_SafetyMetrics_findFirstTrajLaneCollison(vehicleTrajectoryPath, laneMarkerPath, (-1));
...
if 1==0
    ...
    % Call the function
    firstHitStruct = fcn_SafetyMetrics_findFirstTrajLaneCollison(vehicleTrajectoryPath(:,1), laneMarkerPath, (figNum));
    ...
    % Call the function
    firstHitStruct = fcn_SafetyMetrics_findFirstTrajLaneCollison(vehicleTrajectoryPath, laneMarkerPath(:,1), (figNum));
